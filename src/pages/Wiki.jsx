      ...
      <div className="border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/60">
        <div className="max-w-[1500px] mx-auto px-4 sm:px-6 py-6 md:py-8">
          ...
          <div className="flex items-center gap-2 text-xs text-slate-500 dark:text-slate-400 mb-3 font-mono">
            <Link to="/wiki" className="hover:text-slate-900 dark:hover:text-white flex items-center gap-1">
              <BookOpen size={14} className="text-emerald-600 dark:text-emerald-400" /> OMIX Docs
            </Link>
            <ChevronRight size={12} />
            <span className="text-slate-900 dark:text-white font-semibold">{activeCategory ? activeCategory : 'Get Started & Overview'}</span>
          </div>

          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div>
              <h1 className="text-3xl md:text-5xl font-serif font-bold tracking-tight text-slate-950 dark:text-white">
                {activeCategory ? activeCategory : 'OMIX Documentation & System Notes'}
              </h1>
              <p className="mt-2 text-slate-600 dark:text-slate-300 text-sm md:text-base max-w-3xl leading-relaxed">
            ...
            <div className="flex-shrink-0 bg-slate-900 dark:bg-slate-900 text-white p-4 rounded-xl border border-slate-800 shadow-sm max-w-xs">
      ...
      <div className="max-w-[1500px] mx-auto px-4 sm:px-6 py-8 flex flex-col lg:flex-row gap-8">
        ...
        <main className="flex-1 min-w-0">
          ...
          {!category && !searchQuery && (
            <div className="mb-8 rounded-2xl bg-gradient-to-r from-slate-900 via-slate-950 to-slate-900 text-white p-6 md:p-8 border border-slate-800 shadow-lg relative overflow-hidden">
          ...
          {searchQuery && (
            <div className="mb-6 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg p-3 text-sm flex items-center justify-between font-mono">
              <span>Showing search results for: <strong className="text-emerald-600 dark:text-emerald-400">"{searchQuery}"</strong></span>
          ...
          <div className="space-y-4">
            {visibleArticles.length === 0 ? (
              <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-8 text-center">
                <p className="text-slate-600 dark:text-slate-400 font-medium">No documentation found matching your search query.</p>
                <Link to="/wiki" className="mt-3 inline-block text-xs font-bold text-emerald-600 dark:text-emerald-400 hover:underline">
              ...
              visibleArticles.map((article) => (
                <article
                  key={article.slug}
                  className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800/80 rounded-xl p-6 hover:border-slate-400 dark:hover:border-slate-700 hover:shadow-sm transition-all group"
                >
                  <div className="flex flex-wrap items-center gap-3 text-xs text-slate-500 dark:text-slate-400 font-mono mb-2">
                    <span className="font-semibold text-emerald-700 dark:text-emerald-300 bg-emerald-50 dark:bg-emerald-950/60 px-2 py-0.5 rounded border border-emerald-200 dark:border-emerald-800">
                      {article.category}
                    </span>
                    <span>•</span>
                  ...
                  <h2 className="text-xl md:text-2xl font-serif font-bold text-slate-950 dark:text-white mt-1">
                    <Link to={`/wiki/${article.slug}`} className="hover:text-emerald-600 dark:hover:text-emerald-400 transition-colors">
                      {article.title}
                    </Link>
                  </h2>

                  <p className="mt-2 text-slate-600 dark:text-slate-300 text-sm leading-relaxed max-w-3xl">
                    {article.excerpt}
                  </p>
                  ...
                  <div className="mt-4 flex items-center justify-between pt-3 border-t border-slate-100 dark:border-slate-800/60">
                    <Link
                      ...
                      className="inline-flex items-center gap-1.5 text-xs font-bold text-slate-900 dark:text-white group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors"
                    >
                    ...
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="text-[11px] font-mono text-slate-400 hover:text-emerald-600 dark:hover:text-emerald-400 flex items-center gap-1"
                    >
          ...
          {!category && !searchQuery && (
            <section className="mt-12 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 md:p-8">
              <div className="flex items-center gap-2 text-xs font-mono uppercase tracking-wider text-emerald-600 dark:text-emerald-400 font-bold mb-2">
                <CheckCircle2 size={16} /> OMIX Delivery Standard vs Generic Agency
              </div>
              <h2 className="text-2xl font-serif font-bold text-slate-950 dark:text-white mb-4">
                ...
                <table className="w-full text-left text-xs border-collapse">
                  <thead>
                    <tr className="border-b border-slate-200 dark:border-slate-800 text-slate-500 font-mono">
                      ...
                      <th className="py-2.5 px-3 font-bold text-emerald-700 dark:text-emerald-400 bg-emerald-50/50 dark:bg-emerald-950/30">OMIX Systems</th>
                  ...
                  <tbody className="divide-y divide-slate-100 dark:divide-slate-800/60 text-slate-700 dark:text-slate-300">
                    <tr>
                      ...
                      <td className="py-3 px-3 text-emerald-950 dark:text-emerald-200 font-medium bg-emerald-50/30 dark:bg-emerald-950/20">Custom Software, SaaS, APIs & Architecture</td>
                    ...
                    <tr>
                      ...
                      <td className="py-3 px-3 text-emerald-950 dark:text-emerald-200 font-medium bg-emerald-50/30 dark:bg-emerald-950/20">First-class (M-PESA, Stripe, Webhooks, CRMs)</td>
                    ...
                    <tr>
                      ...
                      <td className="py-3 px-3 text-emerald-950 dark:text-emerald-200 font-medium bg-emerald-50/30 dark:bg-emerald-950/20">Modular, decoupled, multi-tenant ready</td>
                    ...
                    <tr>
                      ...
                      <td className="py-3 px-3 text-emerald-950 dark:text-emerald-200 font-medium bg-emerald-50/30 dark:bg-emerald-950/20">Full source ownership, transparent pricing</td>
              ...
              <div className="mt-6 pt-4 border-t border-slate-100 dark:border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-4">
                ...
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="bg-slate-950 dark:bg-slate-100 text-white dark:text-slate-950 hover:bg-slate-800 dark:hover:bg-slate-200 font-semibold text-xs px-4 py-2 rounded-lg transition-colors flex items-center gap-1.5"
                >
