    ...
    <div className="bg-[#f7f8fa] min-h-screen">
      <header className="border-b border-slate-200 bg-white">
        <div className="max-w-6xl mx-auto px-5 py-14">
          ...
          <p className="mt-4 max-w-2xl text-slate-600 text-lg">Engineering notes, product thinking and practical guides from OMIX Systems.</p>
        </div>
      </header>
      <div className="max-w-6xl mx-auto px-5 py-10 grid lg:grid-cols-[220px_1fr] gap-10">
        <aside>
          ...
          <nav className="space-y-1">
            ...
            {categories.map(c => <Link key={c} to={`/category/${encodeURIComponent(c.toLowerCase().replace(/ /g, '-'))}`} className="block rounded-lg px-3 py-2 text-sm text-slate-600 hover:bg-white">{c}</Link>)}
          </nav>
          <div className="mt-10 border-t border-slate-200 pt-6 text-sm text-slate-500">Need a system built?<br /><a className="font-semibold text-slate-900 hover:underline" href="https://omixsystems.com/">Talk to OMIX →</a></div>
        </aside>
        <main>
